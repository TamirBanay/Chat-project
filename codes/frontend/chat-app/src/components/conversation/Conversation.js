...
const Conversation = () => {
  ...
  window.addEventListener('resize', function() {
    const header = document.querySelector('.conversation-profileImgs-and-usernames');
    if (window.innerHeight < 500) { // השתמש בערך שמייצג את גובה המקלדת
      header.style.top = '100px'; // למשל, למקם את ה-header קצת יותר גבוה
    } else {
      header.style.top = '0';
    }
  });
